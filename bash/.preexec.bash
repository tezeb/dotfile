...
function preexec_screen_user_at_host () {
    ...
    then
        ...
        then
            echo -n "@${SCREEN_HOST}" 1>&2;
        else
            echo -n "${USER}@${SCREEN_HOST}" 1>&2;
        fi
    fi
}
...
function preexec_xterm_title_install () {
    ...
    function precmd () {
        ...
        preexec_xterm_title "${PROMPTCHAR} - ${USER}@${SCREEN_HOST} $(dirs -0)"
        ...
        echo -e "\033[38;5;52m ↪ $(date)${rancmd}${showelapsed}\033[0m" 1>&2;
    }
    ...
    function preexec () {
        ...
        echo -e "\033[38;5;23m ↩ $(date)\033[0m" 1>&2;
        prevstart="${cmdstart}";
        cmdstart="$(date '+%s')";
    }
